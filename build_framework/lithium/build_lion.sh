...
/Developer/usr/bin/make distclean
./bootstrap_osx_lion_64.sh
if [ $? -ne 0 ]; then
  echo "ERROR: Induction failed to bootstrap"
  exit 1
fi
PATH=/Developer/usr/bin:/Developer/usr/sbin:$PATH \
/Developer/usr/bin/make -j`sysctl -n hw.ncpu`
if [ $? -ne 0 ]; then
...
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/induction.dSYM" "$FWPREFIX/Libraries/libinduction-$BUILDNUM.0.dylib"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/lithium.dSYM" "$PREFIX/Contents/MacOS/lithium"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/mars.dSYM" "$FWPREFIX/Libraries/lithium/mars.so"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/admin.dSYM" "$FWPREFIX/Libraries/lithium/admin.so"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/client_handler.dSYM" "$FWPREFIX/Libraries/lithium/client_handler.so"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/core.dSYM" "$FWPREFIX/Libraries/lithium/core.so"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/customer.dSYM" "$FWPREFIX/Libraries/lithium/customer.so"
/Developer/usr/bin/dsymutil -o "$DSYM_PATH/device.dSYM" "$FWPREFIX/Libraries/lithium/device.so"
for i in vendor_*
do
  if [ -d $i ]; then
    cd $i
    MOD_NAME=`echo $i | sed s/vendor_//g`
    /Developer/usr/bin/dsymutil -o "$DSYM_PATH/$MOD_NAME.dSYM" "$FWPREFIX/Libraries/lithium/vendor_modules/$MOD_NAME.so"
    cd ..
  fi
done
