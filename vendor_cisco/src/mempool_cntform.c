...
int v_mempool_cntform (i_resource *self, i_entity *ent, i_form_reqdata *reqdata)
{
  ...
  if (cnt->item_list && cnt->item_list_state == ITEMLIST_STATE_NORMAL)
  {
    ...
    item = i_form_table_create (reqdata->form_out, "mempoollist", NULL, 7);
    ...
    labels[2] = "Used";
    labels[3] = "Free";
    labels[4] = "Largest Free";
    labels[5] = "Percent Used";
    labels[6] = "Contiguous Free Percent";
    ...
    for (i_list_move_head(cnt->item_list); (mempool=i_list_restore(cnt->item_list))!=NULL; i_list_move_next(cnt->item_list))
    {
      ...
      labels[1] = i_metric_valstr (mempool->size, NULL);
      labels[2] = i_metric_valstr (mempool->used, NULL);
      labels[3] = i_metric_valstr (mempool->free, NULL);
      labels[4] = i_metric_valstr (mempool->largest_free, NULL);
      labels[5] = i_metric_valstr (mempool->used_pc, NULL);
      labels[6] = i_metric_valstr (mempool->contig_free_pc, NULL);
