...
int v_port_enable (i_resource *self)
{
  ...
  static_cnt = i_container_create ("fcport", "Fibre Ports");
  if (!static_cnt)
  { i_printf (1, "v_port_enable failed to create container"); v_port_disable (self); return -1; }
  static_cnt->mainform_func = v_port_cntform;
  static_cnt->sumform_func = v_port_cntform;

  /* Register entity */
  num = i_entity_register (self, ENTITY(self->hierarchy->dev), ENTITY(static_cnt));
  if (num != 0)
  { i_printf (1, "v_port_enable failed to register container"); v_port_disable (self); return -1; }
  ...
  if (num != 0)
  { i_printf (1, "v_port_enable failed to load and apply container refresh config"); v_port_disable (self); return -1; }
  ...
  { i_printf (1, "v_port_enable failed to create item_list"); v_port_disable (self); return -1; }
  i_list_set_destructor (static_cnt->item_list, v_port_item_free);
  static_cnt->item_list_state = ITEMLIST_STATE_POPULATE;

  static_objfact = l_snmp_objfact_create (self, "port", "Port");
  if (!static_objfact)
  { 
  ...
  static_objfact->name_oid_str = strdup (".1.3.6.1.3.94.1.10.1.11");
