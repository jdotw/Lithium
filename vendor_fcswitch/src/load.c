...
int v_load_enable (i_resource *self)
{
  ...
  if (static_enabled == 1)
  { i_printf (1, "v_load_enable warning, sub-system already enabled"); return 0; }
  ...
  if (!static_cnt)
  { i_printf (1, "v_load_enable failed to create container"); v_load_disable (self); return -1; }
  static_cnt->mainform_func = v_load_cntform;
  static_cnt->sumform_func = v_load_cntform;
  static_cnt->navtree_expand = NAVTREE_EXP_RESTRICT;

  /* Register entity */
  num = i_entity_register (self, ENTITY(self->hierarchy->dev), ENTITY(static_cnt));
  if (num != 0)
  { i_printf (1, "v_load_enable failed to register container"); v_load_disable (self); return -1; }
  ...
  if (num != 0)
  { i_printf (1, "v_load_enable failed to load and apply container refresh config"); v_load_disable (self); return -1; }
  ...
  { i_printf (1, "v_load_enable failed to create item_list"); v_load_disable (self); return -1; }
  static_cnt->item_list_state = ITEMLIST_STATE_NORMAL;

  /* Create object */
  obj = i_object_create ("master", "Master");
  if (!obj)
  { i_printf (1, "v_load_enable failed to create object"); v_load_disable (self); return -1; }
  obj->cnt = static_cnt;
  obj->mainform_func = v_load_objform;
  obj->histform_func = v_load_objform_hist;
  obj->navtree_expand = NAVTREE_EXP_RESTRICT;

  /* Create item */
  load = v_load_item_create ();
  if (!load)
  { i_printf (1, "v_load_enable failed to create item"); v_load_disable (self); return -1; }
  load->obj = obj;
  obj->itemptr = load;
  ...
  i_entity_refreshcb_add (ENTITY(load->one_min_raw), v_load_refcb, NULL);
  ...
  i_entity_refreshcb_add (ENTITY(load->five_min_raw), v_load_refcb, NULL);
  ...
  i_entity_refreshcb_add (ENTITY(load->fifteen_min_raw), v_load_refcb, NULL);
  ...
  if (num != 0)
  { i_printf (1, "v_load_enable warning, failed to apply all triggersets for master object"); }
...
int v_load_disable (i_resource *self)
{
  ...
  if (static_enabled == 0)
  { i_printf (1, "v_load_disable warning, sub-system already disabled"); return 0; }
