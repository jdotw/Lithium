...
int v_load_cntform (i_resource *self, i_entity *ent, i_form_reqdata *reqdata)
{
  /*
  ...
  if (!obj)
  { i_printf (1, "v_load_cntform failed to retrieve master object"); return -1; }
  load = obj->itemptr;
  ...
  if (!item) { i_printf (1, "v_load_formsection failed to create table"); return -1; }
