...
CFLAGS="-gdwarf-2 -arch i386 -W -Wall -Wno-unused-parameter -Werror -Wsign-compare" \
CC="/Developer/usr/bin/clang" \
./configure $CONFIGURE_PATHS --localstatedir='/Library/Application Support/Lithium/Monitoring Data/History' \
--with-curl-includes=$FWPREFIX/Headers \
--with-curl-libs=$FWPREFIX/Libraries \
--with-db-includes=$FWPREFIX/Headers \
--with-db-libs=$FWPREFIX/Libraries \
--with-induction-includes=$FWPREFIX/Headers \
--with-induction-libs=$FWPREFIX/Libraries \
--with-lithium-includes=$FWPREFIX/Headers \
--with-xml-includes=/usr/include/libxml2 \
--with-xml-libs=/usr/lib \
--with-postgres-includes=$DBFWPREFIX/Headers \
