...
int v_voltage_enable (i_resource *self)
{
  int num;
  ...
  static_cnt = i_container_create ("voltage", "Voltages");
  if (!static_cnt)
  { i_printf (1, "v_voltage_enable failed to create container"); return -1; }

  /* Register entity */
  num = i_entity_register (self, ENTITY(self->hierarchy->dev), ENTITY(static_cnt));
  if (num != 0)
  { i_printf (1, "v_voltage_enable failed to register container"); return -1; }
  ...
  if (num != 0)
  { i_printf (1, "v_voltage_enable failed to load and apply container refresh config"); return -1; }
  ...
  { i_printf (1, "v_voltage_enable failed to create item_list"); return -1; }
  i_list_set_destructor (static_cnt->item_list, v_voltage_item_free);
  static_cnt->item_list_state = ITEMLIST_STATE_POPULATE;

  static_objfact = l_snmp_objfact_create (self, "voltage", "voltage");
  if (!static_objfact)
  { 
  ...
  static_objfact->name_oid_str = strdup (".1.3.6.1.4.1.31165.1.1.4.1.1.5");
