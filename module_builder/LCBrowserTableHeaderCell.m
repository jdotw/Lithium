...
- (void) drawInteriorWithFrame:(NSRect)rect inView:(NSView *)controlView
{
	...
	attr = [NSDictionary dictionaryWithObjectsAndKeys:
			[NSColor colorWithCalibratedWhite:0.1 alpha:1.0], NSForegroundColorAttributeName,
						  [NSFont fontWithName:@"Arial Bold" size:10.0], NSFontAttributeName,
						  nil];
	textSize = [[self stringValue] sizeWithAttributes:attr];
...
- (void)drawSortIndicatorWithFrame:(NSRect)rect inView:(NSView *)controlView ascending:(BOOL)ascending priority:(int)priority
{
	if (priority == 0)
	{
		...
		NSBezierPath *path;
		if (ascending) path = (NSBezierPath *)[LCTriangleBezierPath upTriangleInRect:indicatorRect];
		else path = (NSBezierPath *)[LCTriangleBezierPath downTriangleInRect:indicatorRect];
