...
void	UKCrashReporterCheckForCrash(NSWindowController *browser)
{
	if (checked_already) return;
	checked_already = 1;
	...
	NS_DURING
		...
		if( lastTimeCrashLogged )	// We have a crash log file and its mod date? Means we crashed sometime in the past.
		{
			...
			if( [lastTimeCrashReported compare: lastTimeCrashLogged] == NSOrderedAscending )
			{
				...
				[[UKCrashReporter alloc] initWithLogString: currentReport browser:browser];
			}
		}
	NS_HANDLER
...
-(id)	initWithLogString: (NSString*)theLog browser:(NSWindowController *)initBrowser
{
...
-(void)	awakeFromNib
{
	...
	NSString *preferredEmail = [[NSUserDefaults standardUserDefaults] stringForKey:@"UKCrashReporterPreferredEmail"];
	if (preferredEmail)
	{ [self setUserEmail:preferredEmail]; }
	else
	{
	...
	// Show the window:
	[NSApp beginSheet:reportWindow modalForWindow:[browser window] modalDelegate:self didEndSelector:nil contextInfo:nil];
}


-(IBAction)	sendCrashReport: (id)sender
{
	[[NSUserDefaults standardUserDefaults] setObject:[self userEmail] forKey:@"UKCrashReporterPreferredEmail"];
	...
	[crashReportString appendString: [NSString stringWithFormat:@"Email: %@", [self userEmail]]];
	[crashReportString appendString: @"\n\n==========\n\n"];
	...
	// Prepare a request:
	NSString *urlString = [NSString stringWithFormat:@"http://secure.lithiumcorp.com.au/reporter/modb_crash.php?from=%@", [[self userEmail] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
