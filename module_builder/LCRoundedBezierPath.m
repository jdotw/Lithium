...
+ (NSBezierPath *) pathInRect:(NSRect)frame
{
	...
	/* Top right */
	[path curveToPoint:NSMakePoint(NSMaxX(frame)-arcWidth, NSMaxY(frame)) controlPoint1:NSMakePoint(NSMaxX(frame), NSMaxY(frame)) controlPoint2:NSMakePoint(NSMaxX(frame), NSMaxY(frame))];
	
	/* Top line */
	[path lineToPoint:NSMakePoint(x+arcWidth, NSMaxY(frame))];
