//
//  LCBrowserSiteContentController.m
//  Lithium Console
//
...
@implementation LCBrowserSiteContentController

#pragma mark "Constructors"

- (id) initWithSite:(LCSite *)initSite inBrowser:(LCBrowser2Controller *)initBrowser
{
	self = [self initWithNibName:@"SiteContent" bundle:nil];
	...
	if (self)
	{
		...
		self.site = initSite;
		[self.site.customer highPriorityRefresh];
		...
		/* Create device tree */
		self.deviceTree = [[[LCDeviceTree alloc] initWithSite:site] autorelease];
		
		/* Set initial selection */
		self.selectedDevices = [NSArray arrayWithObject:self.site];
		self.inspectorTarget = self.site;
...
- (void) setSelectedDevices:(NSArray *)value
{
	...
	{ 
		...
		self.selectedDevice = [NSArray arrayWithObject:self.site];
	}
}
					   ...
					   context:(void *)context
{
	if (object == deviceTreeController)
	{
		...
		if ([[deviceTreeController selectedObjects] count] > 0)
		{
			...
			else self.inspectorTarget = site;
...
{
	[self.site.customer normalPriorityRefresh];
}
