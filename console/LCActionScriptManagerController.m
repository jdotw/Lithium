//
//  LCActionScriptManagerController.m
//  Lithium Console
//
...
@implementation LCActionScriptManagerController
...
{
	/* Create script list */
	self.scriptList = [LCActionScriptList new];
	self.scriptList.customer = [setupController customer];
	
	/* Refresh list */
	[self.scriptList highPriorityRefresh];
}
...
- (IBAction) uploadScriptClicked:(id)sender
{
	...
	{
		[self setUploadRequest:[[LCScriptUploadRequest uploadScript:[openPanel filename] type:@"action" forCustomer:[setupController customer]] retain]];
...
{
	/* Deletes script */
	LCActionScript *selectedScript = [self selectedScript];
	...
	[rootnode addChild:[NSXMLNode elementWithName:@"type" stringValue:@"action"]];
	...
	selectedScript.installedIcon = [NSImage imageNamed:@"off_16.tif"];
	[scriptList removeObjectFromLocalScriptsAtIndex:[[scriptList localScripts] indexOfObject:selectedScript]];
	[[scriptList scriptDictionary] removeObjectForKey:[selectedScript name]];
}
...
- (IBAction) downloadScriptClicked:(id)sender
{
	...
	LCActionScript *script = [self selectedScript];
...
{
	/* Get selected */
	LCActionScript *selectedScript = [self selectedCommunityScript];
	if (!selectedScript) return;
...
-(void) connectionDidFinishLoading:(NSURLConnection *) connection
{
	...
	[self setUploadRequest:[[LCScriptUploadRequest uploadScript:filename type:@"action" forCustomer:[setupController customer]] retain]];
	[uploadRequest setDelegate:self];
	[uploadRequest performAsyncRequest];
