...
@interface LCGroupTreeCell (private)
...
@implementation LCGroupTreeCell
...
- (void)drawInteriorWithFrame:(NSRect)cellFrame inView:(NSView *)controlView
{
	...
	if ([representedObject class] == [LCGroupTreeOutlineEntityItem class])
	{
		/* Entity item */
		LCGroupTreeOutlineEntityItem *headerItem = representedObject;
		[self drawInteriorWithFrame:cellFrame inView:controlView forEntity:headerItem.entity];
	}
	else if ([representedObject class] == [LCObjectTreeMetricItem class])
	{ 
		/* Metric Item */
