...
@implementation LCObjectTreeOutlineView
...
- (void) awakeFromNib
{
	...
	[super awakeFromNib];
	self.expansionPreferencePrefix = @"LCObjectTreeOutlineView";
...
- (void) dealloc
{
	[displayedSubviews release];
	[super dealloc];
}

#pragma mark "Delegate methods"

- (void)outlineView:(NSOutlineView *)outlineView willDisplayCell:(LCObjectTreeCell *)cell forTableColumn:(NSTableColumn *)tableColumn item:(id)item
{
...
- (void) selectObject:(LCObject *)obj
{
	...
	for (i=0; i < [self numberOfRows]; i++)
	{
		...
		LCObjectTreeItem *objectItem = [item representedObject];
		LCEntity *rowEntity = objectItem.object;
		...
		if (rowEntity == obj)
		{
			...
			if ([self isItemExpanded:[self itemAtRow:i]] && (i+1) < [self numberOfRows])
			{
				[self scrollRowToVisible:i+1];
			}
			return;
		}
		else if ([obj isDescendantOf:rowEntity])
		{
			/* Parent */
			if (![self isItemExpanded:item])
