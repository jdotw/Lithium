...
void i_hashtable_iterate (i_resource *self, i_hashtable *table, void (*funct) (i_resource *self, i_hashtable *res_table, void *data, void *passdata), void *passdata) 
{
  /* Loop through the hash tables entries and run funct with data being cell->data 
   *
   * WARNING : It is NOT safe to make any changes to the hashtable whilst
   * iterating it. This can result in a segfault due to cell->next not really
   * being cell->next after calling the function
   */

  unsigned int i;
  i_hashtable_cell *cell;
  
  if (!self || !table || !funct) return;

  for (i=0; i < table->size; i++)							/* Loop through each cell */
  {
    cell = table->table[i];  

    while (cell)								/* Run up the chain */
    {
      i_hashtable_cell *next;

      next = cell->next;
      funct (self, table, cell->data, passdata);				/* Call the funct */
      cell = next;
    }
  }
}
