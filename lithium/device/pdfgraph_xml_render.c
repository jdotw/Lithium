...
typedef struct l_pdfgraph_entity_s
{
...
l_pdfgraph_entity* l_pdfgraph_entity_create ()
{ 
  l_pdfgraph_entity *g_ent = (l_pdfgraph_entity *) malloc (sizeof(l_pdfgraph_entity));
  memset (g_ent, 0, sizeof(l_pdfgraph_entity));
  return g_ent;
}

void l_pdfgraph_entity_free (void *entptr)
{
  l_pdfgraph_entity *g_ent = entptr;
  if (!g_ent) return;
...
int xml_pdfgraph_render (i_resource *self, i_xml_request *xmlreq)
{
  ...
  i_list_set_destructor (ent_list, l_pdfgraph_entity_free);
  ...
  if (xmlreq->xml_in)
  {
    ...
    for (node = root_node->children; node; node = node->next)
    {
      ...
      { 
        ...
        l_pdfgraph_entity *gent = l_pdfgraph_entity_create ();
        gent->entdesc = i_entity_descriptor_fromxml (xmlreq->xml_in, node);
        if (!gent->entdesc)
        { 
          i_printf (1, "xml_pdfgraph_render failed to convert an entity from xml"); 
          l_pdfgraph_entity_free (gent);
          continue; 
        }
  ...
  {
    i_printf (2, "xml_pdfgraph_render error, no xml data received");
    return -1;
  }
  ...
  switch (period)
  {
    ...
    case GRAPHPERIOD_YEAR:
      ...
      break;
    default:
      i_printf (1, "xml_pdfgraph_render unsupported graph period %i specified; no graph rendered", period);
      return -1;
  }

  /* Loop through entities */
  l_pdfgraph_entity *gent;
  ...
  asprintf (&rrdargs, "--width=550 --height=120 %s %s LINE1:100 LINE1:0", rrddefs, renderargs);
  if (has_percentages)
  { 
    char *tmp;
  ...
  asprintf (&temp_str, "%s.pdf", imagefile);
  free (imagefile);
  imagefile = temp_str;
  ...
  /* Render graph */
  cmd = i_rrd_pdfgraph (self, imagefullpath, start_sec, end_sec, y_label, kbase, rrdargs, l_pdfgraph_xml_render_graphcb, xmlreq);
  free (imagefullpath);
  free (rrdargs);
...
int l_pdfgraph_xml_render_graphcb (i_resource *self, i_rrdtool_cmd *cmd, int result, void *passdata)
{
