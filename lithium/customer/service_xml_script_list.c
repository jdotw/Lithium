  ...
  if (proc->output_str && strstr(proc->output_str, "<?xml"))
  {
    ...
    if (root_node)
    {
      for (node = root_node->children; node; node = node->next)
      {
        ...
        else if (!strcmp((char *)node->name, "info") && str) script->info_str = strdup(str);
        else if (!strcmp((char *)node->name, "version") && str) script->version_str = strdup(str);
        else if (!strcmp((char *)node->name, "proto") && str) script->protocol_str = strdup(str);
        else if (!strcmp((char *)node->name, "transport") && str) script->transport_str = strdup(str);
        else if (!strcmp((char *)node->name, "port") && str) script->port_str = strdup(str);
