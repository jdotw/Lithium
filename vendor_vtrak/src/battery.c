...
int v_battery_enable (i_resource *self, int enclosure, char *oidindex_str)
{
  ...
  char *name_str;
  char *desc_str;
  asprintf (&name_str, "vtbattery_%i", enclosure);
  asprintf (&desc_str, "Enclosure %i Batteries", enclosure);
...
//
  ...
  { 
    ...
    v_battery_disable (self);
    return -1; 
  }
  char *name_oid_str;
  static_objfact->dev = self->hierarchy->dev;
  static_objfact->cnt = static_cnt;
  asprintf (&name_oid_str, ".1.3.6.1.4.1.7933.1.20.1.15.1.1.%s", oidindex_str);
