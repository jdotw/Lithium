...
int v_temp_enable (i_resource *self, int enclosure, char *oidindex_str)
{
  ...
  char *name_str;
  char *desc_str;
  asprintf (&name_str, "vttemp_%i", enclosure);
  asprintf (&desc_str, "Enclosure %i Temperatures", enclosure);
  ...
  i_triggerset_addtrg (self, tset, "warning", "Warning", VALTYPE_STRING, TRGTYPE_NOTEQUAL, 0, "normal", 0, NULL, 120, ENTSTATE_IMPAIRED, TSET_FLAG_VALAPPLY);
  ...
  asprintf (&name_oid_str, ".1.3.6.1.4.1.7933.1.20.1.13.1.1.%s", oidindex_str);
