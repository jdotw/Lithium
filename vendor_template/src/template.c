...
int v_template_enable (i_resource *self, int enclosure, char *oidindex_str)
{
  ...
  char *name_str;
  char *desc_str;
  asprintf (&name_str, "vttemplate_%i", enclosure);
  asprintf (&desc_str, "Enclosure %i Power", enclosure);
  static_cnt = i_container_create (name_str, desc_str);
  free (name_str);
  free (desc_str);
  if (!static_cnt)
  { i_printf (1, "v_template_enable failed to create container"); v_template_disable (self); return -1; }

  /* Register entity */
  num = i_entity_register (self, ENTITY(self->hierarchy->dev), ENTITY(static_cnt));
  if (num != 0)
  { i_printf (1, "v_template_enable failed to register container"); v_template_disable (self); return -1; }
  ...
  if (num != 0)
  { i_printf (1, "v_template_enable failed to load and apply container refresh config"); v_template_disable (self); return -1; }
  ...
  { i_printf (1, "v_template_enable failed to create item_list"); v_template_disable (self); return -1; }
