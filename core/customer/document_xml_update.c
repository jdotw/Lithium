...
int xml_document_update (i_resource *self, i_xml_request *req)
{
  /* Check permission */
  ...
  if (req->xml_in)
  {
    ...
    for (node = root_node->children; node; node = node->next)
    {
      ...
      else if (!strcmp((char *)node->name, "id") && str) doc->id = atoi(str);
  ...
  {
    i_printf (1, "xml_document_update failed, no xml data received");
    return -1; 
  }
  if (doc->type_str == NULL)
  {
    i_printf (1, "xml_document_update failed, no document type specified"); 
    return -1;
  }
  ...
  xmlNodePtr root_node = xmlNewNode (NULL, BAD_CAST "document_update");
  xmlDocSetRootElement (xml->doc, root_node);
  req->xml_out = xml;

  /* Update document */
  int num = l_document_sql_update (self, doc);
  if (num == 0)
  {
