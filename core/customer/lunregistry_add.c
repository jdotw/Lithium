...
{
  /* Add a new entry to the lunregistry tables */
  ...
  if (key)
  {
    ...
    if (!list)
    {
      ...
      { i_printf (1, "l_lunregistry_add warning, failed to create list for device_table"); }
    }
    i_hashtable_free_key (key);
    key = NULL;
    ...
    if (num != 0)
    { i_printf (1, "l_lunregistry_add warning, failed to enqueue entry to device_table list"); }
  }
  else
  { i_printf (1, "l_lunregistry_add warning, failed to create key for device_table"); }
