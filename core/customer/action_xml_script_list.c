...
typedef struct
{
  ...
  int waiting;
  i_list *script_list;
} l_action_script_list_req;

int xml_action_script_list (i_resource *self, i_xml_request *req)
{
  /* Walks the actions directory, queries each script
   * for its description and then returns the xml
   */

  l_action_script_list_req *scriptreq;

  /* Create out requestd ata */
  scriptreq = (l_action_script_list_req *) malloc (sizeof(l_action_script_list_req));
  memset (scriptreq, 0, sizeof(l_action_script_list_req));
  scriptreq->xmlreq = req;

  /* Create list */
  scriptreq->script_list = i_list_create ();
  i_list_set_destructor (scriptreq->script_list, l_action_script_free);

  /* Open dir */
  char *dirname = i_path_glue (self->root, "action_scripts");
  DIR *dir = opendir (dirname);
  if (!dir)
  { i_printf (1, "xml_action_script_list failed to open directory %s", dirname); free(dirname); return -1; }
  free (dirname);
  ...
  while ((dirent=readdir(dir))!=NULL)
  {
    ...
    l_action_proc *proc = l_action_exec (self, dirent->d_name, "info", NULL, NULL, l_action_xml_script_list_scriptcb, scriptreq);
    if (proc)
    {
      scriptreq->waiting++;
...
int l_action_xml_script_list_scriptcb (i_resource *self, l_action_proc *proc, void *passdata)
{
  ...
  l_action_script_list_req *req = passdata;

  /* Decrement counter */
  req->waiting--;

  /* Create script */
  l_action_script *script = l_action_script_create ();
  ...
  if (proc->output_str && strstr(proc->output_str, "<?xml"))
  {
    ...
    i_xml *xml = i_xml_struct (proc->output_str, strlen(proc->output_str), "action_script");
    xmlNodePtr node;
    ...
    if (root_node)
    {
      for (node = root_node->children; node; node = node->next)
      {
        ...
        {
          /* Config variable */
          l_action_configvar *var = l_action_configvar_fromxml (xml, node);
  ...
  if (req->waiting < 1)
  {
    ...
    {      
      xmlNodePtr script_node = l_action_script_xml (script);
