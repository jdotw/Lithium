...
int v_cpu_objform (i_resource *self, i_entity *ent, i_form_reqdata *reqdata)
{
  ...
  str = i_metric_valstr (cpu->fivesec_pc, NULL);
  i_form_string_add (reqdata->form_out, "fivesec_pc", "Five Second Load", str);
  free (str);

  str = i_metric_valstr (cpu->onemin_pc, NULL);
  i_form_string_add (reqdata->form_out, "onemin_pc", "One Minute Load", str);
  free (str);

  str = i_metric_valstr (cpu->fivemin_pc, NULL);
  i_form_string_add (reqdata->form_out, "fivemin_pc", "Five Minute Load", str);
  free (str);
