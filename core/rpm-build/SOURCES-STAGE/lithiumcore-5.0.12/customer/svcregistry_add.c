...
{
  /* Add a new entry to the svcregistry tables */
  ...
  if (key)
  {
    ...
    if (!list)
    {
      ...
      { i_printf (1, "l_svcregistry_add warning, failed to create list for device_table"); }
    }
    ...
    if (num != 0)
    { i_printf (1, "l_svcregistry_add warning, failed to enqueue entry to device_table list"); }
  }
  else
  { i_printf (1, "l_svcregistry_add warning, failed to create key for device_table"); }
