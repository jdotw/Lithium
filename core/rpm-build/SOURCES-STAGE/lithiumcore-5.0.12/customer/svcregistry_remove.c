...
int l_svcregistry_remove (i_resource *self, i_resource_address *devaddr, i_entity_descriptor *objent)
{       
  /* Remove entries from the svcregistry tables */
  ...
  /* Retrieve tables */  
  device_table = l_svcregistry_table_device ();
  obj_table = l_svcregistry_table_object ();
  ...
  if (!key)
  { i_printf (1, "l_svcregistry_remove failed to create key for device_table"); return -1; }
  
  device_entries = i_hashtable_get (device_table, key);
  if (!device_entries)
  { i_printf (1, "l_svcregistry_remove failed, no entries for specified device"); i_hashtable_free_key (key); return -1; }
  ...
  for (i_list_move_head(device_entries); (entry=i_list_restore(device_entries))!=NULL; i_list_move_next(device_entries))
  {
    ...
    if (objent)
    {
      ...
      if (entry->objent && strcmp(entry->objent->name_str, objent->name_str)==0)
