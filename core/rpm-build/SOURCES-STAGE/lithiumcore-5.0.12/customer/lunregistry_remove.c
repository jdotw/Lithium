...
int l_lunregistry_remove (i_resource *self, i_resource_address *devaddr, i_entity_descriptor *objent)
{       
  /* Remove entries from the lunregistry tables */
  ...
  /* Retrieve tables */  
  device_table = l_lunregistry_table_device ();
  obj_table = l_lunregistry_table_object ();
  ...
  if (!key)
  { i_printf (1, "l_lunregistry_remove failed to create key for device_table"); return -1; }
  
  device_entries = i_hashtable_get (device_table, key);
  if (!device_entries)
  { i_printf (1, "l_lunregistry_remove failed, no entries for specified device"); i_hashtable_free_key (key); return -1; }
