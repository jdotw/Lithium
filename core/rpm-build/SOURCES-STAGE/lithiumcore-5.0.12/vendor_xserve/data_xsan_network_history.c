  ...
  switch (opcode & REFOP_ALL)
  {
    case REFOP_REFRESH:     /* Begin the refresh processa */
      if (v_powerstate()) 
      {
        ...
        static_req = v_plist_get (self, url, v_data_xsan_network_history_plistcb, met);
        free (url);
      }
      else return 1;
      break;
...
int v_data_xsan_network_history_plistcb (i_resource *self, v_plist_req *req, void *passdata)
{
...
int v_data_xsan_network_process_samples (i_resource *self, v_plist_req *req, v_xsaninfo_item *info, xmlNodePtr arrayNode)
{
  xmlNodePtr node;
  for (node = arrayNode->children; node; node = node->next)
  {
    if (strcmp((char *)node->name, "dict") == 0)
    {
      ...
      if (!t_str)
      { i_printf (1, "v_data_xsan_network_process_samples no timestamp 't' found"); continue; }
      time_t t_sec = strtoul (t_str, NULL, 10);
      free (t_str);
      t_str = NULL;

      /* Check value */
      if (t_sec == info->last_network_t)
      { i_printf (1, "v_data_xsan_network_process_samples network history has not been updated... skipping"); break; }
      
      /* Get value */
      char *network_str = v_plist_data_from_dict (req->plist, node, "v2");
      if (network_str)
      {
        i_metric_value *val;
        val = i_metric_value_create ();
        val->flt = atol (network_str);
        i_metric_value_enqueue (self, info->network_tput, val);
        info->network_tput->refresh_result = REFRESULT_OK;
        i_entity_refresh_terminate (ENTITY(info->network_tput));
        free (network_str);
        network_str = NULL;
