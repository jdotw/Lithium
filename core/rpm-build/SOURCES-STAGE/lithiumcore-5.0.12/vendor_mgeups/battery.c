...
int v_battery_enable (i_resource *self)
{
  ...
  static_cnt = i_container_create ("mgebattery", "Battery");
  if (!static_cnt)
...
//  battery->lastreplaced = l_snmp_metric_create (self, static_obj, "lastreplaced", "Last Replaced", METRIC_STRING, ".1.3.6.1.4.1.318.1.1.1.2.1.3", "0", RECMETHOD_NONE, 0);
  ...
  /* Advanced */
  battery->capacity = l_snmp_metric_create (self, static_obj, "capacity", "Capacity", METRIC_GAUGE, ".1.3.6.1.4.1.705.1.5.2", "0", RECMETHOD_RRD, 0);
  battery->capacity->record_defaultflag = 1;
  battery->capacity->unit_str = strdup ("%");
  battery->temp = l_snmp_metric_create (self, static_obj, "temp", "Temperature", METRIC_GAUGE, ".1.3.6.1.4.1.705.1.5.7", "0", RECMETHOD_RRD, 0);
  battery->temp->unit_str = strdup ("dec.C");
  battery->temp->record_defaultflag = 1;
  battery->runtime_remaining = l_snmp_metric_create (self, static_obj, "runtime_remaining", "Runtime Remaining", METRIC_GAUGE, ".1.3.6.1.4.1.705.1.5.1", "0", RECMETHOD_NONE, 0);
  battery->runtime_remaining->unit_str = strdup ("sec");
  battery->replace_indicator = l_snmp_metric_create (self, static_obj, "replace_indicator", "Replacement Indicator", METRIC_INTEGER, ".1.3.6.1.4.1.705.1.5.11", "0", RECMETHOD_NONE, 0);
  i_metric_enumstr_add (battery->replace_indicator, 1, "Yes");
  i_metric_enumstr_add (battery->replace_indicator, 2, "No");
