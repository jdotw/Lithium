...
int v_cpu_cntform (i_resource *self, i_entity *ent, i_form_reqdata *reqdata)
{
  ...
  if (cnt->item_list && cnt->item_list_state == ITEMLIST_STATE_NORMAL)
  {
    ...
    item = i_form_table_create (reqdata->form_out, "cpulist", NULL, 4);
    ...
    labels[1] = "Five Sec Load";
    labels[2] = "One Minute Load";
    labels[3] = "Five Minute Load";
    ...
    for (i_list_move_head(cnt->item_list); (cpu=i_list_restore(cnt->item_list))!=NULL; i_list_move_next(cnt->item_list))
    {
      ...
      labels[1] = i_metric_valstr (cpu->fivesec_pc, NULL);
      labels[2] = i_metric_valstr (cpu->onemin_pc, NULL);
      labels[3] = i_metric_valstr (cpu->fivemin_pc, NULL);
