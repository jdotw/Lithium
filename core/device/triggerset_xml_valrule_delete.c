...
int xml_triggerset_valrule_delete (i_resource *self, i_xml_request *xmlreq)
{
  long id = -1;
  char *str;
  char *tset_name = NULL;
  ...
  {
    i_printf (1, "xml_triggerset_valrule_delete error, no xml data received");
    return -1;
  }
  if (!tset_name || !met_name)
  { i_printf (1, "xml_triggerset_valrule_delete no tset_name and/or met_name specified"); return -1; }
  if (id == -1)
  { i_printf (1, "xml_triggerset_valrule_delete no rule ID specified"); return -1; }
  ...
  if (!tset)
  { i_printf (1, "xml_triggerset_valrule_delete failed to find specified trigger set"); return -1; }

  /* Delete */
  i_triggerset_valrule_remove (self, obj, tset, id);

  /* Force a trigger update */
  ...
  root_node = xmlNewNode(NULL, BAD_CAST "triggerset_valrule_update");
  asprintf (&str, "%li", id);
