...
int xml_triggerset_valrule_update (i_resource *self, i_xml_request *xmlreq)
{
  i_object *obj;
  i_triggerset *tset;
  char *tset_name = NULL;
  ...
  {
    i_printf (1, "xml_triggerset_valrule_update error, no xml data received");
    return -1;
  }
  if (!tset_name || !met_name)
  { i_printf (1, "xml_triggerset_valrule_update no tset_name and/or met_name specified"); return -1; }
  ...
  if (!tset)
  { i_printf (1, "xml_triggerset_valrule_update failed to find specified trigger set"); return -1; }

  /* Check rule ID */
  if (rule->id != 0)
  {
    /* Update rule */
    i_triggerset_valrule_update (self, obj, tset, rule);
  }
  else
  {
    /* Insert new rule */
    ...
    { 
      i_triggerset_valrule_add_exclusive (self, obj, tset, rule); 
    }
  }
