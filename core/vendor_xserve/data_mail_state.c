  ...
  switch (opcode & REFOP_ALL)
  {
    case REFOP_REFRESH:     /* Begin the refresh processa */
      if (v_powerstate()) 
      {
        ...
        static_req = v_plist_get (self, url, v_data_mail_state_plistcb, met);
        free (url);
      }
      else return 1;
      break;
...
int v_data_mail_state_plistcb (i_resource *self, v_plist_req *req, void *passdata)
{
  i_metric *datamet = passdata;
  v_services_item *service = v_services_get ("mail");
  ...
  for (node = req->root_node->children; node; node = node->next)
  {
    if (strcmp((char *)node->name, "dict") == 0)
    {
      ...
      /* Process protocols */
      xmlNodePtr protocolsArray = v_plist_node_from_dict (req->plist, node, "protocolsArray");
      if (protocolsArray)
      { v_data_mail_process_protocols (self, req, service, protocolsArray); }
    }
  }
...
int v_data_mail_process_protocols (i_resource *self, v_plist_req *req, v_services_item *service, xmlNodePtr protocolsArray)
{
  /* Process each of the protocols */
  xmlNodePtr node;
  int total_conns = 0;
  for (node = protocolsArray->children; node; node = node->next)
  {
    if (strcmp((char *)node->name, "dict") == 0)
    {
      /* Process a protocol */
      int conn_count = v_plist_int_from_dict (req->plist, node, "active");
  ...
  val->gauge = total_conns;
  i_metric_value_enqueue (self, service->volume, val);
  service->volume->refresh_result = REFRESULT_OK;
