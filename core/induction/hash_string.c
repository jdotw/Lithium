...
unsigned long i_hash_string (unsigned char *k, unsigned long htsize)
{
  ...
  { i_printf (1, "i_hash_string failed, string length is < 1"); return 0; }
  ...
  if (c >= htsize)
  {
    i_printf (1, "HT_FAILURE: i_hash_string calculated invalid hash of %u (htsize is %u). Returning fail-safe hash of 0", c, initval);
    return 0;
  }
