...
%infostruct = (  'version' => '1.0',
                 'desc'  => "POP Service Check",
                 'proto' => "POP",
                 'port'  => "110",
                 'transport' => "TCP",
                 'info' => "Connects to the device using the POP3 protocol
                  and optionally attempts to log in and check for messages",
                  'config_variable'=> [
                              {
                                 'name' => 'test_account',
                                 'desc' => 'Test POP Account'
                              },
                              {
                                'name' => 'test_password',
                                'desc'=> 'Test POP Account Password'
                              }
                 ]
            );
...
sub checkService
{
  ...
  if ($status == $NORMAL)
  {
    ...
    if (is_string($test_account))
    {
      ...
      # Attempt to log in
      $pop = Net::POP3->new($ip, Timeout => 60);
      if ($pop->login($test_account, $test_password) > 0)
      {
        # Connected
        $status = $NORMAL;
