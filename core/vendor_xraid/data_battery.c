...
int v_data_battery_refresh (i_resource *self, i_metric *met, int opcode)
{
  /* Refresh the raw data source for the data_battery data */
  char *action;
  char *plistout;
...
int v_data_battery_plistcb (i_resource *self, v_plist_req *req, void *passdata)
{
  ...
  for (node = req->root_node->children; node; node = node->next)
  {
    if (strcmp((char *)node->name, "dict") == 0)
    {
      ...
      v_battery_item *item;
      if (datamet == dataitem->battery_left)
      { item = v_battery_static_left_item (); }
      else
      { item = v_battery_static_right_item (); }
