...
void module_info ()
{
  ...
  i_printf (0, "Lithium HP Procurve Vendor Module");
}
...
{
  i_printf (0, "hp vendor module_init entered.");
  return 0;
}
...
int module_entry (i_resource *self, i_form *config_form)
{
  ...
  i_printf (0, "hp vendor module_entry entered.");

  num = l_snmp_objfact_init (self, self->hierarchy->dev);
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to initialise l_snmp_objfact"); return -1; }      

  num = l_snmp_sysinfo_enable (self);
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to enable SNMP System Information sub-system"); return -1; }
  
  num = v_cpu_enable (self);
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to enable the memory pool sub-system"); return -1; }

  num = v_sensor_enable (self);
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to enable the sensor sub-system"); return -1; }
  ...
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to enable NW Throughput sub-system"); return -1; }
  
  num = l_snmp_iface_enable (self);
  if (num != 0)
  { i_printf (1, "hp vendor module_entry failed to enable SNMP Interface List sub-system"); return -1; }
...
//    if (num != 0)
//    { i_printf (1, "hp vendor module_entry failed to enable SNMP Routing Table sub-system"); return -1; }
//  }
...
{
  i_printf (0, "hp vendor module_shutdown entered.");
