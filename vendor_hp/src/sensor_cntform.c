...
{
  /*
   * Sensor Table Container Main Form
   */
  ...
  if (cnt->item_list && cnt->item_list_state == ITEMLIST_STATE_NORMAL)
  {
    ...
    i_form_item_add_option (item, ITEM_OPTION_WIDTH, 0, 0, 0, "100%", 4);                 /* 100% Width for main table */
    ...
    for (i_list_move_head(cnt->item_list); (sensor=i_list_restore(cnt->item_list))!=NULL; i_list_move_next(cnt->item_list))
    {
      ...
      labels[2] = i_metric_valstr (sensor->warnings, NULL);
      labels[3] = i_metric_valstr (sensor->failures, NULL);
      
      row = i_form_table_add_row (item, labels);

      i_form_table_add_link (item, 0, row, RES_ADDR(self), ENT_ADDR(sensor->obj), NULL, 0, NULL, 0);
    ...
    i_form_string_add (reqdata->form_out, "sensor_note", "Note", "'*' next to a value indicates the data is not current");
  }
  else
  {
